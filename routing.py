...
class Logger :
	...
	def add_link(cls,link) :
		cls.linkTimes[link] = []
		cls.droppedPacket[link.buffer] = []
		cls.bufferOccupancy[link] = []
...
class Packet:
	...
	def __str__(self) :
		return '<Packet '+self.src+' '+self.dst+' '+str(self.type)+' '+str(self.pktNum)+'>'
...
def test1():
	...
	for i in range(130) :
		...
		print R3.routing_table
		print R4.routing_table
		print 'L0',L0.buffer.size-L0.buffer.available_space
		print 'L1',L1.buffer.size-L1.buffer.available_space
		print 'L2',L2.buffer.size-L2.buffer.available_space
		print 'L3',L3.buffer.size-L3.buffer.available_space
		print 'L4',L4.buffer.size-L4.buffer.available_space
		print 'L5',L5.buffer.size-L5.buffer.available_space
		print ''
...
def test_own(): ##### NOTE: FROM DAVID-- I didnt update the interface for this test
	H1 = Host('H1',30)
	H2 = Host('H2',30)
	R = Router('R',10)
